defmodule Erlangelist.Article do
  ...
  article_meta = fn {article_id, article_spec} ->
    ...
    date = Date.from_iso8601!(article_spec[:posted_at]) |> Timex.to_datetime(:local)

    Enum.into(article_spec, %{
      id: article_id,
      posted_at: Timex.format!(date, "%B %d, %Y", :strftime),
      copyright_year: date.year,
      posted_at_rfc822: Timex.format!(date, "{RFC822}"),
